...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-8">
        ...
        <button onclick="toggleForm()" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-6 rounded-lg transition duration-300 shadow-md">
            + Adicionar Conquista
        </button>
    </div>
    ...
    <div id="achievementForm" class="hidden bg-white rounded-xl shadow-lg p-6 mb-8 border border-gray-100">
        <form action="{{ route('achievements.store') }}" method="POST">
            ...
            <div class="flex justify-end gap-3">
                <button type="button" onclick="toggleForm()" class="px-6 py-2 rounded-lg text-gray-600 hover:bg-gray-100 transition">Cancelar</button>
                <button type="submit" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-8 rounded-lg transition shadow-md">Salvar Conquista</button>
            </div>
        </form>
    </div>
                        ...
                        <div class="bg-white rounded-xl shadow-sm p-5 border border-gray-100 hover:shadow-md transition duration-300">
                            <div class="flex flex-col md:flex-row justify-between items-start gap-4">
                                ...
                                <div class="flex gap-2 flex-shrink-0">
                                    <button onclick='editAchievement(@json($achievement))' class="text-blue-500 hover:bg-blue-50 p-2 rounded-lg transition" title="Editar">
                                    ...
                                    <form action="{{ route('achievements.destroy', $achievement) }}" method="POST" onsubmit="return confirm('Tem certeza?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-500 hover:bg-red-50 p-2 rounded-lg transition" title="Excluir">
...
<div id="editModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-2xl">
        ...
        <form id="editForm" method="POST">
            ...
            <div class="flex justify-end gap-3">
                <button type="button" onclick="closeEditModal()" class="px-6 py-2 rounded-lg text-gray-600 hover:bg-gray-100 transition">Cancelar</button>
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-8 rounded-lg transition shadow-md">Salvar Alterações</button>
            </div>
        </form>
    </div>
</div>
